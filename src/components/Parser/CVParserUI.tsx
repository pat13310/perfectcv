...
const CVParserUI: React.FC<CVParserUIProps> = ({ onParseComplete, onError }) => {
  ...
  const handleFileUpload = async (file: File) => {
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 500)); // Animation visuelle

      // Étape 2: Extraction du texte
      setStatus({ stage: 'extracting', progress: 50 });
      await new Promise(resolve => setTimeout(resolve, 500)); // Animation visuelle

      // Étape 3: Analyse du contenu
      setStatus({ stage: 'analyzing', progress: 75 });
      const result = await CVParser.parseCV(file);
      await new Promise(resolve => setTimeout(resolve, 500)); // Animation visuelle
